...
data_train, data_test = new_data.randomSplit([0.8, 0.2], seed=42)

# Update train_set and test_set tables
data_train.write.mode("append").saveAsTable(f"{catalog_name}.{schema_name}.train_set_an")
data_test.write.mode("append").saveAsTable(f"{catalog_name}.{schema_name}.test_set_an")

# Verify affected rows count for train and test
affected_rows_train = data_train.count()
affected_rows_test = data_test.count()
...
if affected_rows_train > 0 or affected_rows_test > 0 :
    spark.sql(f"""
        WITH max_timestamp AS (
            ...
            FROM {catalog_name}.{schema_name}.train_set_an
        )
...
""")
    spark.sql(f"""
        WITH max_timestamp AS (
            SELECT MAX(update_timestamp_utc) AS max_update_timestamp
            FROM {catalog_name}.{schema_name}.test_set_an
        )
        INSERT INTO {catalog_name}.{schema_name}.features_an
        SELECT pickup_zip, trip_distance
        FROM {catalog_name}.{schema_name}.test_set_an
